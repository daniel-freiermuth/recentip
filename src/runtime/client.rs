...
async fn spawn_udp_subscription_socket<U: UdpSocket>(
    ...
    let socket = match local_port {
        PortSpec::Any => U::bind(SocketAddrV4::new(unicast_ip, 0)).await?,
        PortSpec::Fixed(port) => U::bind(SocketAddrV4::new(unicast_ip, port)).await?,
        /*
    ...
    let local_endpoint = SocketAddrV4::new(unicast_ip, bound_addr.port());
...
/// Handle `Command::Subscribe` for UDP transport
///
/// This handler processes UDP subscriptions synchronously (inline in the event loop)
/// since UDP socket binding is instant and doesn't block.
///
...
/// receive which event.
pub async fn handle_subscribe_udp<U: UdpSocket>(
    service_id: crate::ServiceId,
    instance_id: crate::InstanceId,
    major_version: u8,
