...
use someip_runtime::wire::{
    Header, SdMessage, PROTOCOL_VERSION, PROTOCOL_VERSION_OFFSET, 
    INTERFACE_VERSION_OFFSET, SD_METHOD_ID, SD_SERVICE_ID,
};
        ...
        for _ in 0..20 {
            ...
            if let Ok(Ok((len, _from))) = result {
                ...
                let mut response = build_request_with_protocol_version(
                    0x1234, 0x0001, header.client_id, header.session_id,
                    0x01, 0x01, b"ok"
                );
...
fn server_ignores_wrong_protocol_version() {
    ...
    sim.host("server", || async {
        ...
        assert!(result.is_err(), "Server should not receive requests with wrong protocol version");
                ...
                if let Some((_header, sd_msg)) = parse_sd_message(&buf[..len]) {
                    for entry in &sd_msg.entries {
                        if entry.entry_type as u8 == 0x01 && entry.service_id == 0x1234 {
                            ...
                            assert_eq!(
                                entry.major_version, TestService::MAJOR_VERSION,
...
fn response_preserves_interface_version() {
    ...
    sim.client("raw_client", async move {
        ...
        let request = build_request_with_protocol_version(
            0x1234, 0x0001, 0xABCD, 0x1234,
            0x01, 0x01, // interface version 1
            b"test"
        );
