...
use someip_runtime::{EventId, EventgroupId, InstanceId, MethodId, RuntimeConfig, Service, ServiceId};
...
fn test_find_service() {
    ...
    sim.host("client", || async {
        let config = RuntimeConfig::default();
        let runtime: TurmoilRuntime = Runtime::with_socket_type(config)
            .await
            .unwrap();
...
fn test_method_call_rpc() {
    ...
    sim.host("client", || async {
        ...
        ).await.expect("Timeout waiting for response").expect("Call should succeed");
...
fn test_event_subscription() {
    ...
    sim.host("client", || async {
        ...
        ).await.expect("Timeout subscribing").expect("Subscribe should succeed");
        
        // Wait for events
        let event1 = tokio::time::timeout(
            Duration::from_secs(5),
            subscription.next()
        ).await.expect("Timeout waiting for event1");
        ...
        let event2 = tokio::time::timeout(
            Duration::from_secs(5),
            subscription.next()
        ).await.expect("Timeout waiting for event2");
